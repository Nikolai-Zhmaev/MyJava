...
//            resetVertexStates();
        ...
        @Override
        public void minDistance(String startVertexLabel, String finishVertexLabel) {

            Vertex vertex = find(startVertexLabel);
            Vertex finVertex = find(finishVertexLabel);
            Queue<Vertex> queue = new ArrayDeque();

            if (vertex == null || finVertex == null) {
                return;
            }

            if (vertex == finVertex) { // обработаем частный случай
                System.out.println(finVertex);
                return;
            }
            else {
                   ...
                   while (!queue.isEmpty()) {
                       ...
                       while ((currentVertex = getAdjUnvisitedVertex(vertex)) != null) {
                           currentVertex.setPreVertex(vertex); // сохраним предыдущий объект

                           if (currentVertex == finVertex) { // остановим обход при нахождении нужного элемента
                              resetVertexStates();
                               Stack<Vertex> stack = new Stack(); // создадим стек для вывода кротчайшего пути.
                                // заполним наш стек
                               visit(currentVertex, stack);
                               vertex = currentVertex.getPreVertex();
                               while (vertex != null) {
                                   visit(vertex, stack);
                                   vertex = vertex.getPreVertex();
                               }
                               //выведем наш стек
                               System.out.println("Кротчайшее растояние: ");
                               while (!stack.isEmpty()) {
                                   System.out.print(" - ");
